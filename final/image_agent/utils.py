...
class SuperTuxDataset(Dataset):
    def __init__(self, dataset_path='my_data.pkl', transform=ToTensor()):
        from PIL import Image
        # from glob import glob
        # from os import path
        self.data = []
        for f in load_pickle(dataset_path):
            x = f['soccer_state']['ball']['location']
            y = f['team1_state'][0]['camera']['projection'].T
            z = f['team1_state'][0]['camera']['view'].T
            puck = to_image(x, y, z)
            im = f['team1_images'][0]
            im = Image.fromarray(im)
            self.data.append((im, puck))
...
def load_data(dataset_path=DATASET_PATH, transform=ToTensor(), num_workers=0, batch_size=128):
